...
;init serial uart
		RCALL USART_Init
		ser temp
		sts AfficheSerialFlag, temp	;enable serial display in message sub routine
;init afficheur
		rcall reset_afficheur
		rcall videecran
		rcall posi1
		ldi r31,high(data7*2)  		;" Lars DIY GPSDO "    Ici on va chercher l'addresse mémoire ou se retrouve le data à afficher.
		ldi r30,low(data7*2)		;Load adress of message in Z register
		rcall message
		rcall nextline
		rcall posi2
		ldi r31,high(data8*2)  		;"counter yt 1.03"
		ldi r30,low(data8*2)
		rcall message
		rcall nextline
		rcall tempo5s
...
;init 16 bits pwm. No need for the Lars counter but was needed for me to adjust my ocxo to do testing. Disabled
		ldi temp,$FF		;set le top			******************* Always send High first after Low
		sts icr1h, temp		;					
		ldi temp, $FF		;FFFF = 65535 pour 16 bit de resolution.
		sts icr1l, temp
		ldi temp, (1<<COM1A1)|(1<<WGM11)|(0<<WGM10)	;mode 14 fast pwm top = ICR1
		sts tccr1a, temp
		ldi temp, (1<<WGM12)|(1<<WGM13)|(0<<CS10)	;no prescaler   -----> 0<<CS10 = pwm OFF / 1<<CS10 = pwm ON
		sts tccr1b, temp
...
;Track push button on pd3. If this one is pressed at startup eeprom is erase.
...
;*********************************************************************************************************************************
ytyt:
;set overflow of timer0
		ldi temp, (1<<TOIE0)	;enable interupt overflow
		sts timsk0, temp
		ldi temp, (0<<CS00)		;stop counter 8 bit
		out TCCR0B,temp
		clr temp	
		out tcnt0, temp		;met le compteur a 0 au cas
watchdogsetup:	;enable int watchdog 1 secondes. Si le gps pulse manque un pulse. Le watchdow timeout et on passe en mode selfrunning
...
;***********************************************************************************************************************************
runmodeprep1:
		rcall videecran ;clear lcd display screen
runmodeprep:
...
Gate10ks:
		clr temp
		sts AfficheSerialFlag, temp	;disable serial display on message sub routine
		rcall affichegate10ks	;Gate 10000s on lcd only
		rcall espace
		rcall affichesatellite2
		ldi temp,$10	;time gate second in Hex. 0x2710 = 10000
		sts echantillon_timel, temp	;to have 10 seconds ----­> 0x000A
		ldi temp,$27
...
Gate1ks:
		clr temp
		sts AfficheSerialFlag, temp ;disable serial display on message sub routine
		rcall affichegate1ks	;Gate 1000s on lcd only
		rcall espace
		rcall espace
		rcall affichesatellite2
		ldi temp,$e8	;time gate second in Hex. 0x03e8 = 1000
		sts echantillon_timel, temp	;to have 10 seconds ----­> 0x000A
		ldi temp,$03
		sts echantillon_timeh,temp
		rjmp Retour_en_mode_interrupt
...
;*****************************************************************************************************************************************
;*****************************************************************************************************************************************
;Come here each time tcnt0 overflow.
...
;ferme le led counter
		...
		cbr temp, 0b01000000
		out portd, temp
;active serial diaplay
		ser temp
		sts AfficheSerialFlag, temp
...
displaymhz1000:						;j'ai   01 00 00 00 00 00  ou  00 99 99 99 99 99
		ser temp
		sts AfficheSerialFlag, temp	;enable serial display messages
		rcall posi1
		rcall affichegate1ks	;Gate 1000s on lcd and serial
		clr temp
		sts AfficheSerialFlag, temp	;disable serial display messages
		rcall espace
		rcall espace
		rcall virguleserial
		rcall affichesatellite2serial
		rcall virguleserial
...
displaymhz10000:				;10 00 00 00 00 00   ou   09 00 00 00 00 00
		...
		sts AfficheSerialFlag, temp	;enable serial display messages
		rcall posi1
		rcall affichegate10ks	;Gate 1000s on lcd and serial
		clr temp
		sts AfficheSerialFlag, temp	;disable serial display messages
		rcall espace
		rcall virguleserial
...
Retour_en_mode_interrupt:
		ldi temp, (0<<int1)|(0<<int0)	;deactive int0 interrup gps pulse
...
pasencorepret2:
		...
		store eifr,temp
		ldi temp, (0<<int1)|(1<<int0)	;active int0 interrup gps pulse
